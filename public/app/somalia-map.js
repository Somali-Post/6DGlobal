...
(function() {
    ...
    // --- MODIFIED: This function now uses the simpler, local-first geocoding ---
    function processLocation(latLng) {
        const locationData = getAuthoritativeLocation(latLng);
        if (!locationData) {
            console.warn("Could not determine district for the selected point. Click is likely in a gap between polygons.");
            return;
        }
    ...
    // --- This is now our single source of truth for the address ---
